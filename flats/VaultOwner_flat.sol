...
// File: contracts/MakiVault.sol

pragma solidity ^0.6.12;

contract MakiVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 makiAtLastUserAction; // keeps track of maki deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IHRC20 public immutable token; // Maki token
    IHRC20 public immutable receiptToken; // Sugar token
    ...
    /**
     * @notice Constructor
     * @param _token: Maki token contract
    ...
    /**
     * @notice Deposits funds into the Maki Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in MAKI)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.makiAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests MAKI tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
     ...
     * @return Expected reward to collect in MAKI
     */
    function calculateHarvestMakiRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingMaki(0, address(this));
     ...
     * @return Returns total pending maki rewards
     */
    function calculateTotalPendingMakiRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingMaki(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the Maki Vault
